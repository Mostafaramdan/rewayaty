...
@section ('content')
    ...
    <div  id="alert">
        ...
        <div class="flex-grow-1"></div>
              <div class="m-2">
                <!-- <button class="btn btn-primary px-5 add" onClick="event.preventDefault();" data-toggle="modal" data-target="#addEdit-new-modal"> ADD New Provider <i class="ml-2 fas fa-plus-circle"></i></button> -->
              </div>
          </div>
        <div class="table-responsive">
          <table class="table bg-light mb-4 tableInfo" id="tableInfo" dir="rtl">
            @include('dashboard.providers.tableInfo')
          </table>
        </div>
      ...
      <!-- Large modal -->
      @include('dashboard.providers.viewModal')
      <!-- end Large modal -->

      <!-- addEdit new modal -->
        @include('dashboard.providers.addEditModal')
      <!-- end add user modal -->
  ...
  $("body").on("click",".get-record",function(){
    ...
    $.ajax({
      ...
      success: function(record) {
        ...
        $(".view-modal .storeName").html(record.store[0].name);
      }
    });
  });
