    ...
    <div  id="alert">
        ...
        <div class="table-responsive">
          <table class="table bg-light mb-4 tableInfo" id="tableInfo" dir="rtl">
            @include('dashboard.sliders.tableInfo')
          </table>
        </div>
      ...
      <!-- Large modal -->
      @include('dashboard.sliders.viewModal')
      <!-- end Large modal -->

      <!-- addEdit new modal -->
        @include('dashboard.sliders.addEditModal')
      <!-- end add user modal -->
      ...
      success: function(record) {
        ...
        for (var k in record) {
          if (record.hasOwnProperty(k)) {
            if(k.includes('image')  ){
              if(record[k]){  
                $('img#'+k).attr('src', "{{url('/')}}"+record[k]).attr("hidden",false);
              }else{
