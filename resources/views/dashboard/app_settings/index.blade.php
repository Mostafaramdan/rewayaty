    ...
    <div  id="alert">
      ...
      <div class="form-row">
        <div class="flex-grow-1"></div>
              <div class="m-2">
                <button class="btn btn-primary px-5 settings" onClick="event.preventDefault();" data-toggle="modal" data-target="#addEdit-new-modal"> تعديل  <i class="ml-2 fas fa-plus-circle"></i></button>
              </div>
          </div>
        <div class="table-responsive">
          <table class="table bg-light mb-4 tableInfo" id="tableInfo" dir="rtl">
            @include('dashboard.app_settings.tableInfo')
          </table>
        </div>
    </div>
      @include('dashboard.app_settings.viewModal')
      @include('dashboard.app_settings.addEditModal')
</div>
@push('script')
<script>
      ...
      success: function(record) {
        ...
        for (var k in settings) {
          if (settings.hasOwnProperty(k)) { 
              $(".addEdit-new-modal input[name='"+k+"']").val(settings[k]);
               $(".addEdit-new-modal textarea[name='"+k+"']").val(settings[k]);
          }
        }
        $('#emails').val('');
        for(let k in emails){
          $('#emails').tagsinput('add',emails[k]["email"]);
        }
      }
    });
  });
