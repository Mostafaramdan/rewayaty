        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form  id="createUpdate" action="{{route('dashboard.'.Request::segment(2).'.createUpdate')}}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="dropdown ">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <input class="form-control" name='search_for_user' type='search' placeholder="ابحث بالايميل وبالتليفون" >
                                @foreach(\App\Models\users::where('is_active',1)->forPage(1,10)->get() as $record)
                                    <a class="dropdown-item" href="#" val="{{$record->id}}" data-name="{{$record->name}}">  {{$record->name}}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row mr-10" >
                        <div class="form-group col-md-12">
                            <button class="btn btn-primary " onClick="event.preventDefault();$(this).parents('.row').find('input:file').click();">اختر صورة الغلاف <i class="fas fa-image"></i></button>
                        </div>
                        <div class="col-md-12">
                            <input type="file" id="img"  accept="image/*" hidden data-image="cover_image" >
                            <input type="hidden"  name="cover_image" hidden  >
                            <img id="cover_image" class="img-thumbnail" hidden style="border-radius: 50%;height: 50%;max-width: 50%;max-height: 200px;min-height: 200px;"/>
                            <hr/>
                        </div>
                    </div>
                    <div class="form-group" >
