...
<script>
        ...
        for (var k in record) {
          if (record.hasOwnProperty(k)) {
            if(k.includes('image')  ){
              ...
              if(record.is_super_admin==1){
                $(".addEdit-new-modal input[name='is_super_admin']").attr('checked',true);
                $(".permissions").addClass("d-none");
              }else{
                  $(".addEdit-new-modal input[name='is_super_admin']").attr('checked',false);
  ...
  $("body").on("click",".addEdit-new-modal input[name='is_super_admin']",function(){
    if(this.checked)
      $(".permissions").addClass("d-none");
    else{
      $(".permissions").removeClass("d-none");
