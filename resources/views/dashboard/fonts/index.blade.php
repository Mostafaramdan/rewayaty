...
@section ('content')
    ...
    <div  id="alert">
        ...
        <div class="flex-grow-1"></div>
              <div class="m-2">
                <button class="btn btn-primary px-5 add" onClick="event.preventDefault();" data-toggle="modal" data-target="#addEdit-new-modal"> إضافة  Font <i class="ml-2 fas fa-plus-circle"></i></button>
              </div>
          </div>
        <div class="table-responsive">
          <table class="table bg-light mb-4 tableInfo" id="tableInfo" dir="rtl">
            @include('dashboard.fonts.tableInfo')
          </table>
        </div>
      ...
      <!-- Large modal -->
      @include('dashboard.fonts.viewModal')
      <!-- end Large modal -->

      <!-- addEdit new modal -->
        @include('dashboard.fonts.addEditModal')
      <!-- end add user modal -->
...
<script>
  ...
  // 
  document.getElementById('choose-font').onchange=function(){
    let selectedOption = this.options[this.selectedIndex];
    let link = document.createElement('link');
    link.href=`${selectedOption.getAttribute('font-url').replaceAll('http','https')}`;
    link.rel= 'stylesheet';
    document.getElementsByTagName('head')[0].append(link)

    // console.log(fontName);
    // document.getElementById('example-font').css.fonFamily= fontName;
  }
</script> 
@endpush
@endsection
