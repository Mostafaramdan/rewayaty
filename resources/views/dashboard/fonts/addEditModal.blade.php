            ...
            <div class="modal-body">
                <form  id="createUpdate" action="{{route('dashboard.'.Request::segment(2).'.createUpdate')}}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <select  id = "choose-font" class="selectpicker" data-hide-disabled="true" data-live-search="true" name='name'>
                            @foreach($fonts as $font)
                                <option font-url="{{ $font['files']['regular']??null }}"  value="{{ $font['family'] }}">{{ $font['family'] }}</option>
                            @endforeach
                    </select>
                    <span id='example-font'>هذا مثال لشكل الخط</span>
                    </div>
                    <div class="form-group">
