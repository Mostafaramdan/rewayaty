...
use App\Models\report_persons as model;

class report_persons extends Controller
{
public static $model;
...
public static function index()
{
    ...
    return view('dashboard.report_persons.index',compact("records","totalPages",'currentPage'));
}   
...
public static function indexPageing(Request $request)
{
    ...
    if($request->search){
        ...
        $records= $records->filter(function($item) use ($search) {
                if( stripos($item['name'],$search) !== false ||  stripos($item['reports_types']['name'],$search) !== false)
                    return true;
                return false;
            });
    }
    ...
    $tableInfo= (string) view('dashboard.report_persons.tableInfo',compact('records'));
