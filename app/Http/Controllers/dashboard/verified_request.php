...
use App\Models\verified_request as model;

class verified_request extends Controller
{
public static $model;
...
public static function index()
{
    ...
    return view('dashboard.verified_request.index',compact("records","totalPages",'currentPage'));
}   
...
public static function indexPageing(Request $request)
{
    ...
    if($request->search){
        ...
        $records= $records->filter(function($item) use ($search) {
                if( stripos($item['users']['name'],$search)!== false || stripos($item['users']['phone'],$search)!== false || stripos($item['users']['email'],$search)!== false )
                    return true;
                else 
                    return false;
            });
    ...
    $tableInfo= (string) view('dashboard.verified_request.tableInfo',compact('records'));
