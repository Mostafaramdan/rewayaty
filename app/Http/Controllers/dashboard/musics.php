...
use App\Models\musics as model;
...
class musics extends Controller
{
    ...
    {
        $records= self::$model::orderBy('id','DESC')->get();
        ...
        return view('dashboard.musics.index',compact("records","totalPages",'currentPage'));
    }   
    ...
    public static function indexPageing(Request $request)
    {
      ...
      $records= self::$model::all()->$sort($request->sortBy??"id",);    if($request->search){
        ...
        $tableInfo= (string) view('dashboard.musics.tableInfo',compact('records'));
    ...
    public static function createUpdate(Request $request)
    {
        ...
        $messagesAr=[
            ...
            "musicFile.required_if"     =>"يجب ادخال ملف الموسيقي",
